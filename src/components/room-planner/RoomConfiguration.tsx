...
export function RoomConfiguration({
  ...
  return <div className="animate-fade-up">
      ...
      <div className="grid gap-4 md:grid-cols-2 mb-6">
        ...
        <div className="rounded-xl bg-primary/10 border-2 border-primary/30 p-5">
          <div className="flex items-start gap-3">
            ...
            <div>
              <p className="font-medium text-base mb-1">King rooms are fixed</p>
        ...
        <div className="rounded-xl bg-primary/10 border-2 border-primary/30 p-5">
          <div className="flex items-start gap-3">
            ...
            <div>
              ...
              <p className="text-sm text-muted-foreground">Upstairs, accessed through the kitchen. Their shared bathroom is slightly smaller. </p>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="grid gap-4 md:grid-cols-3 mb-6">
        <RoomTypeCard title="King rooms (fixed)" description="Rooms 1 & 6 are fixed as King beds (en-suite bathrooms)." image={roomKingImage} quantity={2} isLocked={true} />
        
        <RoomTypeCard title="Queen size bed rooms" description="Select how many flexible rooms should be prepared with 1 queen bed." image={roomQueenImage} quantity={roomSelection.queenRoomsQty} maxQuantity={remainingFlexible + roomSelection.queenRoomsQty} onIncrement={() => onSetQueenRooms(roomSelection.queenRoomsQty + 1)} onDecrement={() => onSetQueenRooms(roomSelection.queenRoomsQty - 1)} />
        
        <RoomTypeCard title="Twin beds rooms" description="Select how many flexible rooms should be prepared as 2 single beds." image={roomTwinsImage} quantity={roomSelection.twinsRoomsQty} maxQuantity={remainingFlexible + roomSelection.twinsRoomsQty} onIncrement={() => onSetTwinsRooms(roomSelection.twinsRoomsQty + 1)} onDecrement={() => onSetTwinsRooms(roomSelection.twinsRoomsQty - 1)} />
      </div>

      {/* Validation Error */}
      {!isSelectionValid && <div className="rounded-xl bg-destructive/10 border border-destructive/30 p-4 mb-6">
