...
// custom offer — on organise plusieurs taxis ou un minibus via les drivers.
...
const Transportation = () => {
  ...
  const standardTripPrice = useMemo(() => {
    ...
    if (newTrip.passengers_count > STANDARD_RATE_MAX_PASSENGERS) return null;
      ...
      <div className="max-w-3xl mx-auto space-y-6">
        ...
        <div className="guest-card p-6">
          <div className="flex items-start gap-5">
            <div className="w-24 h-24 md:w-32 md:h-32 rounded-full overflow-hidden flex-shrink-0 bg-muted border-4 border-[#EEF1E4]">
              <img 
                src={driverImage} 
                alt="Luis" 
        ...
        {!isLocked && (
          <>
            {showAddTrip ? (
                ...
                <CardContent className="space-y-4">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-muted-foreground mt-1">
                      {newTrip.passengers_count > STANDARD_RATE_MAX_PASSENGERS
                        ? 'For groups of 9 or more, we’ll arrange several taxis or a minibus and include it in your custom offer.'
                        : 'We’ll arrange the right vehicle for your group.'}
                    </p>
              ...
              <Button onClick={() => setShowAddTrip(true)} variant="outline" className="w-full gap-2 h-12 rounded-2xl border-dashed text-muted-foreground hover:text-[#6D7855] hover:border-[#8CA05F]/50 hover:bg-[#EEF1E4]/50">
      ...
      <CardContent className="p-5">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 gap-x-4 gap-y-3 text-sm mb-4">
          ...
          <div>
            ...
            <span className="font-semibold text-[#6D7855] tabular-nums">
...
function EditTripForm({
  ...
  const editedStandard = useMemo(() => {
    ...
    if (!pickup || !dropoff) return null;
    if (form.passengers_count > STANDARD_RATE_MAX_PASSENGERS) return null;
    ...
    <Card className="guest-card border-border/70 shadow-none">
      ...
      <CardContent className="space-y-4">
        ...
        <div>
          ...
          />
          <p className="text-xs text-muted-foreground mt-1">
            {form.passengers_count > STANDARD_RATE_MAX_PASSENGERS
              ? 'For groups of 9 or more, we’ll arrange several taxis or a minibus and include it in your custom offer.'
              : 'We’ll arrange the right vehicle for your group.'}
          </p>
        </div>
