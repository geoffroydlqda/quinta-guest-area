...
- **Emails automatiques configurables (23 août 2026)** : tables `email_rules` (regles creees depuis l admin : trigger `check_in`/`check_out`/`due_date`, `offset_days` négatif = avant, filtre event_type optionnel, sujet/corps avec variables `{{first_name}} {{name}} {{retreat_name}} {{check_in_date}} {{check_out_date}} {{amount}} {{label}} {{due_date}}`, bouton `none`/`guest_area`/`pay`) et `email_rule_log` (dédoublonnage par `dedup_key` unique = `rule|booking|installment` — jamais deux envois pour le même trio). Edge Function `email-rules-run` (`verify_jwt=false`, auth x-cron-key OU JWT admin) : `{run:true}` envoie, `{preview:true}` dry-run, `{test:{rule_id,to}}` envoie un exemple rendu (préfixe [TEST], jamais journalisé). Cron quotidien `email-rules-daily` à 08:05 UTC. UI : onglet **Emails** dédié `/admin/emails` (`AutomatedEmailsCard` — pas que les paiements : infos pré-arrivée, feedback form, etc. ; bouton `custom` = `cta_label` + `cta_url`) — liste + switch enable, éditeur, aperçu du jour, test vers sa propre adresse. Règles créées désactivées par défaut. Exclusions : bookings test, annulés, emails `internal+`. Le bouton Pay réutilise le lien signé HMAC de `payment-emails` (session Stripe créée au clic) ; sur un trigger check-in/out il vise la prochaine échéance impayée.
